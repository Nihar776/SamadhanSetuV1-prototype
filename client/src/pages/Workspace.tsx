...
import { ArrowRight, Bell, Building2, Camera, Check, CheckCircle2, ClipboardCheck, FileText, LogOut, MapPin, Mic, Send, ShieldCheck, Sparkles, ThumbsUp, Users, AlertTriangle, Flag, BadgeCheck } from "lucide-react";
...
export default function Workspace() {
  ...
  const [communityVotes, setCommunityVotes] = useState<Record<number, { upvotes: number; verified: boolean; voted: boolean; alsoFace: boolean; reported: boolean }>>({});

  const communityIssues = [
    { id: 1, title: "Irregular drinking water supply in Ward 7", reporter: "Ramesh K.", pin: "834001", category: "Water Supply", timeAgo: "2 hours ago", upvotes: 14, threshold: 20, description: "Handpump near primary school has not been working for 3 days. Children are forced to carry water from 2km away.", verified: false },
    { id: 2, title: "Broken street lights on NH-33 service road", reporter: "Sunita D.", pin: "834001", category: "Infrastructure", timeAgo: "5 hours ago", upvotes: 23, threshold: 20, description: "All 6 street lights between Doranda Chowk and Lalpur are non-functional. The stretch is dangerous at night.", verified: true },
    { id: 3, title: "Open drainage overflow near market area", reporter: "Amit P.", pin: "834001", category: "Sanitation", timeAgo: "1 day ago", upvotes: 8, threshold: 20, description: "Sewage overflow on the main road near vegetable market causing health hazards and blocking pedestrian path.", verified: false },
    { id: 4, title: "Delayed garbage collection in residential block", reporter: "Priya M.", pin: "834001", category: "Sanitation", timeAgo: "3 days ago", upvotes: 18, threshold: 20, description: "No garbage collection for the past week in Block C. Waste is piling up near the community park.", verified: false },
  ];

  const handleUpvote = (id: number) => {
    setCommunityVotes((prev) => {
      const current = prev[id] || { upvotes: 0, verified: false, voted: false, alsoFace: false, reported: false };
      if (current.voted) return prev;
      const issue = communityIssues.find((i) => i.id === id)!;
      const newUpvotes = issue.upvotes + current.upvotes + 1;
      const nowVerified = newUpvotes >= issue.threshold;
      if (nowVerified) toast.success("Community threshold reached! Issue is now Verified.");
      return { ...prev, [id]: { ...current, upvotes: current.upvotes + 1, voted: true, verified: nowVerified } };
    });
  };

  const handleAlsoFace = (id: number) => {
    setCommunityVotes((prev) => {
      const current = prev[id] || { upvotes: 0, verified: false, voted: false, alsoFace: false, reported: false };
      if (current.alsoFace) return prev;
      toast.success("Your confirmation has been recorded.");
      const issue = communityIssues.find((i) => i.id === id)!;
      const newUpvotes = issue.upvotes + current.upvotes + 1;
      const nowVerified = newUpvotes >= issue.threshold || current.verified;
      return { ...prev, [id]: { ...current, upvotes: current.upvotes + 1, alsoFace: true, verified: nowVerified } };
    });
  };
  ...
  const handleReport = (id: number) => {
    setCommunityVotes((prev) => {
      const current = prev[id] || { upvotes: 0, verified: false, voted: false, alsoFace: false, reported: false };
      if (current.reported) return prev;
      toast.message("This issue has been flagged for review.");
      return { ...prev, [id]: { ...current, reported: true } };
    });
  };
    ...
    <main className="min-h-screen bg-[#f4f6ee] text-[#19332b]">
      <header className="border-b border-[#d5e0d3] bg-[#fffdf6]">
        <div className="mx-auto flex h-[74px] max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-10">
          <Link href="/" className="flex items-center gap-3 rounded-md"><img src="/images/logo-mark.png" className="h-10 w-10" alt="SamadhanSetu symbol" /><span><span className="block font-bold tracking-[-0.02em]">SamadhanSetu</span>
<span className="block text-[11px] font-semibold uppercase tracking-[0.12em] text-[#6b7b73]">{detail.workspace}</span></span></Link>
      ...
      <div className="mx-auto max-w-[1320px] px-4 py-8 sm:px-6 lg:px-10 lg:py-11">
        ...
        {role === "citizen" && <>
          ...
          <section className="mt-7 rounded-[26px] border border-[#d5e0d3] bg-[#fffdf6] p-5 shadow-[0_12px_30px_rgba(29,56,44,0.07)] sm:p-8">
            <div className="flex flex-col justify-between gap-5 md:flex-row md:items-start">
              <div>
                <div className="flex items-center gap-2">
                  <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#eaf2e6] text-[#176b4d]"><Users className="h-5 w-5" /></span>
                  <p className="eyebrow">Community Polling</p>
                </div>
                <h2 className="mt-4 font-display text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Issues in your area</h2>
                <p className="mt-3 max-w-xl text-sm leading-6 text-[#587066]">Citizens sharing PIN code <span className="rounded-lg bg-[#eaf2e6] px-2 py-0.5 font-extrabold text-[#176b4d]">{session.profile.area || "834001"}</span> can upvote, verify, or report issues below. Once enough neighbours confirm an issue, it becomes <strong className="text-[#176b4d]">Verified</strong>.</p>
              </div>
              <div className="flex items-center gap-3 rounded-2xl border border-[#d1ddce] bg-[#f7faf3] px-4 py-3">
                <BadgeCheck className="h-5 w-5 text-[#176b4d]" />
                <div><p className="text-xs font-bold uppercase tracking-[0.1em] text-[#6b7b73]">Verification threshold</p><p className="text-lg font-extrabold text-[#193f31]">20 upvotes</p></div>
              </div>
            </div>

            <div className="mt-7 space-y-4">
              {communityIssues.map((issue) => {
                const votes = communityVotes[issue.id] || { upvotes: 0, verified: false, voted: false, alsoFace: false, reported: false };
                const totalUpvotes = issue.upvotes + votes.upvotes;
                const isVerified = issue.verified || votes.verified || totalUpvotes >= issue.threshold;
                const progressPercent = Math.min((totalUpvotes / issue.threshold) * 100, 100);

                return (
                  <article key={issue.id} className={`rounded-[20px] border p-5 transition-all ${isVerified ? "border-[#176b4d]/30 bg-[#f0f7ee]" : "border-[#d6e0d4] bg-white"} ${votes.reported ? "opacity-60" : ""}`}>
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${isVerified ? "bg-[#176b4d] text-white" : "bg-[#fff5df] text-[#966114]"}`}>
                            {isVerified ? <><BadgeCheck className="h-3.5 w-3.5" /> Verified</> : "Under review"}
                          </span>
                          <span className="rounded-full bg-[#eef3ea] px-3 py-1 text-xs font-bold text-[#3d6553]">{issue.category}</span>
                          <span className="text-xs text-[#8a9a92]">{issue.timeAgo}</span>
                        </div>
                        <h3 className="mt-3 text-lg font-bold text-[#244d40]">{issue.title}</h3>
                        <p className="mt-1.5 max-w-2xl text-sm leading-6 text-[#66786f]">{issue.description}</p>
                        <p className="mt-2 text-xs text-[#8a9a92]">Reported by <strong className="text-[#4d665b]">{issue.reporter}</strong> · PIN {issue.pin}</p>
                      </div>
                    </div>

                    {/* Progress bar toward verification threshold */}
                    <div className="mt-4">
                      <div className="flex items-center justify-between text-xs font-bold">
                        <span className="text-[#5a7066]">{totalUpvotes} of {issue.threshold} upvotes</span>
                        <span className={isVerified ? "text-[#176b4d]" : "text-[#966114]"}>{Math.round(progressPercent)}%</span>
                      </div>
                      <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-[#e5eee1]">
                        <div className={`h-full rounded-full transition-all duration-500 ${isVerified ? "bg-[#176b4d]" : "bg-gradient-to-r from-[#e5a53a] to-[#d4882a]"}`} style={{ width: `${progressPercent}%` }} />
                      </div>
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      <button
                        ...
                        disabled={votes.voted || votes.reported}
                        className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-bold transition ${votes.voted ? "border-[#176b4d]/30 bg-[#eaf2e6] text-[#176b4d]" : "border-[#c6d4c5] bg-white text-[#3d6553] hover:border-[#94b89b] hover:bg-[#f0f7ee]"} disabled:cursor-not-allowed`}
                      >
                        <ThumbsUp className="h-4 w-4" /> {votes.voted ? "Upvoted" : "Upvote"}
                      </button>
                      <button
                        onClick={() => handleAlsoFace(issue.id)}
                        disabled={votes.alsoFace || votes.reported}
                        className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-bold transition ${votes.alsoFace ? "border-[#176b4d]/30 bg-[#eaf2e6] text-[#176b4d]" : "border-[#c6d4c5] bg-white text-[#3d6553] hover:border-[#94b89b] hover:bg-[#f0f7ee]"} disabled:cursor-not-allowed`}
                      >
                        <CheckCircle2 className="h-4 w-4" /> {votes.alsoFace ? "Confirmed" : "I also face this"}
                      </button>
                      <button
                        onClick={() => handleReport(issue.id)}
                        disabled={votes.reported}
                        className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-bold transition ${votes.reported ? "border-[#d97706]/30 bg-[#fff5df] text-[#966114]" : "border-[#d6c6b4] bg-white text-[#8a6d4b] hover:border-[#d97706] hover:bg-[#fff8e8]"} disabled:cursor-not-allowed`}
                      >
                        <Flag className="h-4 w-4" /> {votes.reported ? "Reported" : "Report fake / duplicate"}
                      </button>
                    </div>
                  </article>
                );
            ...
            <div className="mt-6 rounded-xl border border-[#ecd6a3] bg-[#fff7e6] p-4 text-sm leading-6 text-[#75521a]">
              <div className="flex items-start gap-3">
                <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-[#d97706]" />
                <p><strong>How verification works:</strong> When an issue reaches the community upvote threshold (20 upvotes), or a government official manually approves it, the status automatically updates to <strong className="text-[#176b4d]">Verified</strong>. Verified issues are prioritized for resolution.</p>
              </div>
            </div>
          </section>
        </>}
